import { IssueReactionRecord } from "../../types/record.js";
import { loadJson } from "../../io/load-json.js";
import { writeProcessedSISO } from "../../io/write-processed.js";
import { daysBetween } from "../utils/date.js";

const processName = "issue-reactions";

export async function process(): Promise<void> {
  const raw = await loadJson<IssueReactionRecord[]>(
  ...
  const stats = raw.map((reaction) => {
    ...
    const issueKey = `${reaction.repo}#${reaction.issue_number}`;
    ...
    return {
      ...
      issueNumber: reaction.issue_number,
  ...
  const topReactedIssues = [...issueStats].slice(0, 25).map((issue) => ({
  ...
  await writeProcessedSISO(processName, {
    ...
    overview: {
      ...
      uniqueIssues: issueStats.length,
